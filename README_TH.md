...
| [เริ่มต้นใช้งานด่วน](#เริ่มต้นใช้งานด่วน-quick-start) | [setup.py](#setuppy-การตั้งค่าเริ่มต้น) | [sensor_lib.py](#sensor_libpy-การรับรู้และเซนเซอร์) | [ข้อมูลอ้างอิง API](#ข้อมูลอ้างอิง-api) |
...
การเขียนโค้ดสำหรับหุ่นยนต์จะทำที่ไฟล์ `main.py` หรือไฟล์ภารกิจในโฟลเดอร์ `missions/` เป็นหลัก โดยคุณสามารถเรียกใช้คำสั่งพื้นฐานได้ทันที ดังตัวอย่างนี้:

```python
import system.pid_lib as P

async def mission_1(odo, tm):
    # 1. เดินหน้าตรง 50 เซนติเมตร (หุ่นจะค่อยๆ เร่งและเบรกให้เองอย่างนุ่มนวล)
    await P.straight(odo, distance_cm=50)

    # 2. เลี้ยวหันหน้าไปที่มุม 90 องศา
    await P.turn(odo, target_heading=90)

    # 3. สั่งให้แขนยกทำงาน พร้อมกับเดินหน้าไป 20 เซนติเมตร ในเวลาเดียวกัน
...
|---|---|---|
| `straight` | `odo, dist_cm (เซนติเมตร), vmax` | เดินตรงโดยใช้ Odometry และ Gyro ควบคุมทิศทางพร้อมการทำ S-Curve |
| `turn` | `odo, target_h (องศา), vmax` | หมุนตัวไปยังทิศทางสัมบูรณ์ (Absolute Heading) ด้วย Gyro PID |
| `pivot_turn` | `odo, target_h (องศา), side` | เลี้ยวแบบล็อกล้อข้างใดข้างหนึ่งไว้กับที่ (`side='left'` หรือ `'right'`) |
| `swing_turn` | `odo, target_h (องศา), outer_speed, inner_ratio` | เลี้ยวแบบโค้งกว้างโดยใช้ความเร็วล้อซ้าย-ขวาต่างกัน |
| `arc` | `odo, radius_cm (เซนติเมตร), angle_deg (องศา), vmax` | วิ่งเลี้ยวโค้งตามรัศมีและองศาที่กำหนด |
| `goto_xy` | `odo, tx (ซม.), ty (ซม.), vmax` | สั่งให้หุ่นยนต์เดินทางไปยังพิกัดเป้าหมาย (X, Y) บนสนามโดยตรง |
...
|---|---|---|---|
| `track_line` | `pid_lib` | `odo, dist_cm (เซนติเมตร), vmax, sensor_port, edge` | เดินตามเส้น 1 เซ็นเซอร์โดยอัปเดตพิกัด Odometry ไปด้วยในตัว |
| `lf_pd` | `sensor_lib` | `dist_cm (เซนติเมตร), vmax, p` | เดินตามเส้น 1 เซ็นเซอร์ธรรมดาด้วยระบบ PD ตามระยะทาง |
| `lf_gyro` | `sensor_lib` | `dist_cm (เซนติเมตร), vmax, p` | เดินตามเส้น 1 เซ็นเซอร์ร่วมกับ Gyro ช่วยทรงตัวในความเร็วสูง |
| `lf_dual` | `sensor_lib` | `dist_cm (เซนติเมตร), vmax` | เดินตามเส้นโดยใช้เซ็นเซอร์คู่ (คุมเส้นตรงกลาง) |
| `lf_dual_gyro`| `sensor_lib` | `dist_cm (เซนติเมตร), vmax` | เดินตามเส้นด้วยเซ็นเซอร์คู่ + Gyro (เร็วและเสถียรที่สุด) |
| `lf_n_junctions`| `sensor_lib` | `n (จำนวนทางแยก), vmax, mode` | เดินตามเส้นไปเรื่อยๆ จนเจอทางแยก แล้วหยุดที่ทางแยกที่กำหนด |
| `until_line` | `sensor_lib` | `vmax, heading (องศา)` | วิ่งตรงตามทิศทางที่กำหนดจนกว่าเซ็นเซอร์จะเจอเส้นดำ |
...
| `motor_to_angle` | `port, target_deg (องศา), speed` | ปรับตำแหน่งแขนยกไปยังองศาแบบสัมบูรณ์ (Absolute) ด้วย PID |
| `motor_run_until_stall` | `port, speed` | สั่งให้มอเตอร์หนีบหรือกดค้างไว้จนกว่าจะเจอวัตถุชน (มอเตอร์ติดขัด) |
| `motor_run_time` | `port, speed, duration_ms (มิลลิวินาที)` | สั่งให้มอเตอร์หนุนตามเวลาที่กำหนดแล้วหยุดเบรก |
| `straight_with_motor` | `odo, dist_cm (เซนติเมตร), vmax, arm_port, arm_target_deg (องศา)` | วิ่งตรงไปด้วยและขยับปรับตำแหน่งแขนยกไปด้วยในเวลาเดียวกัน |
